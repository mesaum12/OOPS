...
bool check_availability_of_book(int book_id,int serial_num,char *fn)
{
    ...
    while(!f.eof())
    {
        ...
        if(b.return_book_id()==book_id && b.return_copy()->return_serial_number()==serial_num)
        {
            f.close();
            return true;
        }
...
int return_total_faculty()
{
    int temp;
    fstream f;
    f.open("facultylist.txt",ios::binary);
    f.seekg(0,ios::end);
...
{
    fstream f("booklist.txt",ios::in|ios::ate|ios::binary);
    BOOK b;
     while(!f.eof())
    {
       
       f.read((char*)&b,sizeof(BOOK));
       if(b.return_book_id()==t.ret_book_id() && b.return_copy()->return_serial_number()==t.ret_serial_number())
       {
...
int main()
{
    ...
    fl.open("facultylist.txt",ios::app|ios::binary);
    sl.open("studentlist.txt",ios::app|ios::binary);
    bl.open("booklist.txt",ios::app|ios::binary);
    BOOK b;faculty f;
    ...
    student s;
     do
     {
      ...
      if(temp==1)break;
      b.set_details();
      bl.write((char*)&b,sizeof(BOOK));
     } while (1);
    //initialilse the faculty list and store the list in the text file facultylist.txt
     do
     {
   ...
   //block 1 begins here
    if(t.ret_issue_or_return()==0 && flag==1)
    {
       ...
       {
          fl.open("facultylist.txt",ios::in|ios::ate|ios::binary);
          faculty f;
          while(!fl.eof())
          {
             ...
             if(f.return_member_id()==t.ret_member_id() && f.check_eligibility_faculty())
             {
                 ...
                 tl.open("transactionlist.txt",ios::app|ios::binary);
                 tl.write((char *)&tl,sizeof(Transaction));
                 tl.close();//closing the transaction file after we have appended the value
       ...
       //block 1.2 begins
        ...
        {
            sl.open("studentlist.txt",ios::in|ios::ate|ios::binary);
            student s;
            while(!sl.eof())
            {
    ...
    //block 2 begins
     else if(t.ret_issue_or_return()==1 && flag)
      {
         ...
         {
           Transaction tr;
           tl.open("transaction.txt",ios::in|ios::out|ios::binary);
           while(!tl.eof())
           {
