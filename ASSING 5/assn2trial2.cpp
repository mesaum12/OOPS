...
class UniqueCopy
{
    ...
    {
        cout<<"Enter the serial number for the current book id:";
        cin>>serial_number;
    }
    void get_no_of_copies()
    {
        cout<<"Enter the no of copies for the current book id: ";
        cin>>no_of_copies;
    }
...
class BOOK
{
   ...
   void set_details()
   {
       ...
       cin>>book_id;
       cout<<"Enter the total count of serial numbers available for this book id:";
   ...
   int return_serial_present(int serial_no)
   {
       for(int i=0;i<count_of_avl_serial_nums;i++)
       {
           if(Cb[i].return_serial_number()==serial_no && Cb[i].return_number_of_copies()>0)
           return 1;
       }
       return 0;
   }
...
class TransactionList
{
    ...
    public:
    TransactionList(int n=0)
    {
        //just a custom overloaded constructor to allow TransactionList t ,
    }
...
class Booklist
{
   ...
   void update_book_details(Transaction &t){
     for(int i=0;i<number;i++)
     {
         ...
         {
             for(int j=0;j<book_list[i].return_avail_serial_count();j++)
             {
   ...
   void update_book_details_return(Transaction &t)
   {
       for(int i=0;i<number;i++)
       {
           if(book_list[i].return_book_id()==t.ret_book_id())
           {
               UniqueCopy *ptr=book_list[i].return_copy();
               int temp=book_list[i].return_avail_serial_count();
...
int main()
{
   ...
   else if(t.ret_issue_or_return()==0 && flag)
   {
